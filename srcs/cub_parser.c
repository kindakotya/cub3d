...
/*   Updated: 2021/04/16 03:48:47 by gmayweat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_all(int fd, t_args *s_args, char check[8])
{
	...
	i = 0;
	if (check[0] == -1)
		perror("Error: Resolution is wrong.\n");
	if (check[1] == -1)
		perror("Error: Path to the north texture is wrong.\n");
	if (check[2] == -1)
		perror("Error: Path to the south texture is wrong.\n");
	if (check[3] == -1)
		perror("Error: Path to the east texture is wrong.\n");
	if (check[4] == -1)
		perror("Error: Path to the west texture is wrong.\n");
	if (check[5] == -1)
		perror("Error: Path to the sprite is wrong.\n");
	if (check[6] == -1)
		perror("Error: Floor color is wrong.\n");
	if (check[7] == -1)
